...
@SuppressWarnings({"ConstantValue", "FieldCanBeLocal", "unused"})
public class CompatibilityUtilities {

    private final Logger logger;

    private final boolean IS_DEBUG = true; // Should not be set to true on release build. If it is "true" on GitHub, please do tell me in Issues
    private final boolean ADVANCED_DEBUG = false; // Will not matter if IS_DEBUG is false

    public CompatibilityUtilities(Logger logger) {
